...
async function loadModelUrl(value){
  const target=new URL(value,location.href);
  if(target.origin!==location.origin||!target.pathname.toLowerCase().endsWith('.psd'))throw new Error('同一サイトのPSD URLだけ読み込めます');
  return loadModel(async signal=>{
    const r=await fetch(target.href,{signal});
    if(!r.ok)throw new Error('PSDを取得できません（HTTP '+r.status+'）');
    if(Number(r.headers.get('Content-Length'))>RT.MAX_FILE_BYTES)throw new Error('PSDは128MB以下にしてください');
    return r.arrayBuffer();
...
function deform(L, e){
  ...
  for(let k=0;k<n;k+=2){
    ...
    // Skirt fabric stays attached at the waist and moves progressively toward the hem.
    if(bn==='bottomwear'){
      const hem=smooth((y-L.y)/Math.max(1,L.h));
      const side=clamp((x-bcx)/Math.max(1,L.w*0.5),-1,1);
      const wave=(e.skirtIdle||0)*(0.35+0.65*Math.abs(side));
      x+=wave*(10.5+3.5*side)*FS*hem*hem;
      y+=Math.abs(wave)*2.0*FS*hem;
    }
    // A combined legwear layer still needs two-legged motion. Keep the hips fixed
    // and rotate each half in opposite phase, with the displacement growing down
    // toward the ankles. This is intentionally small enough to preserve the pose.
    if(bn==='legwear'){
      const down=smooth((y-L.y)/Math.max(1,L.h));
      const side=x<bcx?-1:1;
      const hipX=bcx+side*L.w*0.16, hipY=L.y+L.h*0.08;
      const th=side*(e.legIdle||0)*0.018*down;
      const ct=Math.cos(th), st=Math.sin(th), rx=x-hipX, ry=y-hipY;
      x=hipX+rx*ct-ry*st+side*(e.legIdle||0)*2.2*FS*down*down;
      y=hipY+rx*st+ry*ct;
    }
    // --- bang blocks ---
...
function animate(now,dt){
  ...
  e.skirtIdle=auto.idle?Math.sin(t*0.92+1.1)*0.55+e.body*0.45:0;
  e.legIdle=auto.idle?Math.sin(t*0.72+0.2)*0.72:0;
