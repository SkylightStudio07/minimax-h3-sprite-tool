...
def build_showcase(folder: Path, output: Path | None = None, *, max_size: int = 640,
    ...
    def paste_legs(target: Image.Image, record: dict, image: Image.Image, angle: float) -> None:
        """Animate a combined legwear image as two halves while keeping hips planted."""
        midpoint = image.width // 2
        overlap = max(1, round(image.width * 0.015))
        for side, bounds, direction in (
            (-1, (0, 0, min(image.width, midpoint + overlap), image.height), 1.0),
            (1, (max(0, midpoint - overlap), 0, image.width, image.height), -1.0),
        ):
            piece = image.crop(bounds)
            pivot_x = piece.width * (0.68 if side < 0 else 0.32)
            piece = piece.rotate(angle * direction, Image.Resampling.BICUBIC, expand=False,
                                 center=(pivot_x, piece.height * 0.08))
            left = round(float(record["left"]) * scale) + bounds[0]
            top = round(float(record["top"]) * scale)
            target.alpha_composite(piece, (left, top))
    ...
    for frame_index in range(frame_count):
        ...
        for layer_key in layer_order:
            ...
            if image is not None and opacity > 0.001:
                if part_id == "legwear":
                    paste_legs(composed, part, image, math.sin(phase * 0.72 + 0.2) * 0.8)
                else:
