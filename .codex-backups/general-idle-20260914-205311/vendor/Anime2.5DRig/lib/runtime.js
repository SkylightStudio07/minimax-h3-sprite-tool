...
(function(root, factory) {
  ...
  function idleProfile(parts, anchors, canvasHeight) {
    const bottom=Math.max(...parts.map(p=>p.top+p.height),1);
    ...
    return {height,bottom,waist,cx,
      coatBottom:clothing&&clothing.top+clothing.height>waist+height*0.22?clothing.top+clothing.height:0};
  }
  const ease=v=>{v=clamp(v,0,1);return v*v*(3-2*v);};
  function idleOffset(id,x,y,p,phase) {
    const h=p.height;
    let dx=0,dy=-h*0.0045*(0.5+0.5*Math.sin(phase))*ease((p.bottom-y)/(h*0.65));
    if(id==='topwear'&&p.coatBottom){
      const u=clamp((y-p.waist)/Math.max(1,p.coatBottom-p.waist),0,1);
      const side=clamp((x-p.cx)/(h*0.22),-1,1);
      const wave=Math.sin(phase-side*0.65-u*1.8)+0.24*Math.sin(phase*2-side-u*3.2);
      dx+=h*0.016*u*u*wave;
      dy+=h*0.0035*u*u*Math.sin(phase-side*0.65-u*1.8+0.8);
    }
