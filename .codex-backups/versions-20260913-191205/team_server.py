...
def create_app(data_dir=None, runner=None, rigging_runner=None):
    ...
    def public_gallery():
        ...
        for row in rows:
            ...
            if approved:
                ...
                if p.get('jobType')=='rigging': items[-1].update(composite=result.get('composite'),psd=result.get('psd'),package=result.get('package'),packageV2=result.get('packageV2'),rigRevision=result.get('rigRevision',0),rigV2BoneCount=result.get('rigV2BoneCount'),player=result.get('player'))
    ...
    def rig_editor_payload(jid, row, asset_prefix, readonly=False):
        ...
        for part in manifest.get('parts',[]):
            asset=f'{asset_prefix}/{part["id"]}'
            parts.append({key:part.get(key) for key in ('id','runtimeName','sourceLayerName','left','top','width','height')}|
                         {'asset':asset,'originalAsset':asset+'?original=1'})
        return dict(jobId=jid,revision=revision,canvas=manifest['canvas'],parts=parts,readonly=readonly,
    ...
    def save_rig_revision(jid,p,row):
        ...
        try:
            ...
            if int(latest_result.get('rigRevision',0))!=expected:
                return jsonify(error='다른 창에서 결과가 수정되었습니다. 새로고침한 뒤 다시 편집하세요.'),409
            folder=(rigging.RESULT_ROOT/jid).resolve()
            saved=rig_edit.save_revision(folder,p.get('operations'),expected,ROOT/'tools'/'write_psd.js')
