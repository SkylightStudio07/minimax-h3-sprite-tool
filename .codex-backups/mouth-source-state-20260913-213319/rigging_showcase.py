...
def build_showcase(folder: Path, output: Path | None = None, *, max_size: int = 640,
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        expression_images = {
            ...
            for key, item in expressions.items() if key in ("eye_open_original", "eye_close", "mouth_close", "eyebrow")
        }

    eye_indices = [index for index, part in enumerate(parts)
                   if str(part.get("id", "")).startswith(EYE_PREFIXES)]
    mouth_indices = [index for index, part in enumerate(parts) if part.get("id") == "mouth_open"]
    eye_insert = max(eye_indices, default=-1)
    mouth_insert = max(mouth_indices, default=-1)
    ...
    for frame_index in range(frame_count):
        ...
        # The base art uses the slightly open mouth. It closes softly once per loop.
        mouth_close = max(0.0, math.sin(phase - 0.35)) ** 8 * 0.72
        ...
        for index, part in enumerate(parts):
            ...
            if index == mouth_insert and "mouth_close" in expression_images:
                paste_record(composed, expressions["mouth_close"], expression_images["mouth_close"], mouth_close)
